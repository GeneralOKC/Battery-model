...
def unpack_vars(U, Mp, Ms):
    ...
    phis_pe = dynamic_slice(U, [phisp0], [phisp0+Mp+2 - phisp0])
    phie_sep = dynamic_slice(U, [phiesep0],[phiesep0+Ms+2-phiesep0])
    # phis_ne = dynamic_slice(U, [phisn0], [phisn0 + Mn + 2 - phisn0])

    j_pe = dynamic_slice(U, [jp0], [jp0+Mp - jp0])
    # j_ne = dynamic_slice(U, [jn0], [jn0 + Mn - jn0])

    return phis_pe, phie_sep, j_pe

@partial(jax.jit, static_argnums=(1,2,3,))
def unpack_fast(U, Mp, Np, Ms):
...
#    eta_ne = U[etan0 : etan0+ Mn]
    ...
    uvec_pe = dynamic_slice(U, [up0], [usep0-up0])
    uvec_sep = dynamic_slice(U, [usep0], [usep0+Ms+2-usep0])
    # uvec_ne = dynamic_slice(U, [un0], [un0+Mn+2 - un0])
    ...
    eta_pe =dynamic_slice(U,[etap0],[etap0+Mp-etap0])
    # eta_ne =dynamic_slice(U,[etan0],[etan0+Mn-etan0])
    
    return uvec_pe, uvec_sep, phie_pe, phie_sep, phis_pe, j_pe, eta_pe
