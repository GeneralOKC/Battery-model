...
def p2d_init_fast(Np, Mp, Ms, delta_t, T, cathode, electrolyte):
    solver_fast = ResidualFunctionFast(Mp, Np, Ms,delta_t, T, cathode, electrolyte)
...
#        gamma_n = gamma_n*np.ones(Mn)
        ...
        val = solver_fast.res_u_pe(val, uvec_pe, jvec_pe, uvec_old_pe, uvec_sep)
        val = solver_fast.res_u_sep(val, uvec_sep, uvec_old_sep, uvec_pe, Icell)
