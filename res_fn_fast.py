...
class ResidualFunctionFast():
    def __init__(self, Mp, Np, Ms, delta_t, T, cathode, electrolyte):
        self.Mp = Mp
        self.Np = Np
        self.Ms = Ms
        self.T = T
    ...
    def res_u_pe(self, val, uvec, jvec, uvec_old, uvec_sep):
        up0 = self.up0
        Mp = self.Mp
        peq = self.peq
    ...
    @partial(jax.jit, static_argnums=(0,))
    def res_u_sep(self, val, uvec, uvec_old, uvec_pe, Icell):
        usep0 = self.usep0
        Ms = self.Ms
        peq = self.peq
        Mp = self.Mp
