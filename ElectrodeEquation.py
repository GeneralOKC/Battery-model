...
from coeffs import electrolyteDiffCoeffs, electrolyteConductCoeffs, solidConductCoeff, solidDiffCoeff, open_circuits
...
#        bc = (c1-c0) + jvec*hy/Deff
    ...
    def electrolyte_conc(self,un, uc, up, j,uold):
        ...
        ans = (uc-uold) - (self.delta_t/eps)*( ( Deff_r*(up - uc)/hx - Deff_l*(uc - un)/hx )/hx + a*(1-self.trans)*j ) 
...
#        bc = -Deff_pe*(u1_pe - u0_pe)*sep.hx + Deff_sep*(u1_sep - u0_sep)*pe.hx
    ...
    def electrolyte_poten(self,un, uc, up, phien, phiec, phiep, j):
        ...
        #                                        np.log(un))/hx )/hx
        ans = a*F*j + (kapeff_r*(phiep - phiec)/hx- kapeff_l*(phiec - phien)/hx)/hx - ((D_r*F/self.trans*(up-uc)/hx)-D_l*F/self.trans*(uc-un)/hx)/hx
        return ans.reshape()
